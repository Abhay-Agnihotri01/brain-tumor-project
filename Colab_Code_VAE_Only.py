# %% [markdown]
# # Standalone VAE Model for Real-Time Brain Tumor Detection
# This notebook implements a standalone VAE model evaluating metrics (Accuracy, Dice, Inference Time, Model Size).
# We use `MedMNIST` (PneumoniaMNIST) as a binary classification proxy (Normal vs. Abnormal).
...
class VAEOnly(nn.Module):
    def __init__(self, latent_dim=128):
        ...
        self.decoder = DecoderModule(latent_dim)
...
def get_model_size_mb(model):
    torch.save(model.state_dict(), "temp_vae.pt")
    size_mb = os.path.getsize("temp_vae.pt") / (1024 * 1024)
    os.remove("temp_vae.pt")
    return size_mb
...
# ---------------------------------------------------------
EPOCHS = 25
vae_model = VAEOnly().to(device)
opt_vae = optim.Adam(vae_model.parameters(), lr=2e-4, weight_decay=1e-5)

bce_loss = nn.BCELoss()
mse_loss = nn.MSELoss()

print("Training Standalone VAE...")
vae_train_losses = []
vae_test_losses = []
for epoch in range(EPOCHS):
    ...
    for imgs, labels in train_loader:
        ...
        opt_vae.zero_grad()
        recon, mu, logvar, real_preds = vae_model(imgs)
        ...
        loss = rec_loss + 0.1 * kl_loss + cls_loss
        loss.backward()
        opt_vae.step()
        epoch_loss += loss.item()
    vae_train_losses.append(epoch_loss / len(train_loader))
    
    # Calculate Test Loss Proxy
    vae_model.eval()
    t_loss = 0
    with torch.no_grad():
        for imgs, labels in test_loaders['Primary Test']:
            ...
            kl_loss = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp()) / imgs.size(0)
            cls_loss = bce_loss(real_preds, labels)
            t_loss += (rec_loss + 0.1 * kl_loss + cls_loss).item()
    vae_test_losses.append(t_loss / len(test_loaders['Primary Test']))

    print(f"Epoch {epoch+1}/{EPOCHS} - Train L: {vae_train_losses[-1]:.4f} | Test L: {vae_test_losses[-1]:.4f}")
...
with torch.no_grad():
    for ds_name, loader in test_loaders.items():
        ...
        for imgs, labels in loader:
            ...
            # Test images pass through the full pipeline to get the classification
            _, _, _, pred = vae_model(imgs)
...
plt.plot(vae_train_losses, label='Training Loss', color='#8E44AD', lw=2)
plt.plot(vae_test_losses, label='Testing Loss', color='#F5A623', lw=2)
plt.title('Training and Testing Loss Curves - VAE Only', fontsize=14)
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
...
bar_colors = ['#8E44AD', '#E67E22', '#2980B9'][:len(accuracies)]
plt.bar(list(eval_results.keys()), accuracies, color=bar_colors)
plt.title('Accuracy Comparison Across Datasets: VAE Only', fontsize=14)
plt.ylabel('Accuracy (%)')
...
print(f"\nModel Size:       {get_model_size_mb(vae_model):.2f} MB")
print(f"Inference Time:   {evaluate_inference_time(vae_model):.4f} s/img")

# ---------------------------------------------------------
# 7. SAVE AND DOWNLOAD MODEL (FOR COLAB)
# ---------------------------------------------------------
print("\nSaving final model...")
MODEL_PATH = "vae_model_final.pth"
